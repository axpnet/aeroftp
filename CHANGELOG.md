...
### MEGA 2FA modal hookup, TOTP QR code, mouse Back button

Same-day patch on top of v3.6.8. The "Two-Factor Authentication prompt modal" advertised in v3.6.8 was wired only into the `connectToFtp` catch handler, so it never opened when the user clicked a saved MEGA card with a stale session — the user landed back on the connection grid with a generic "auth failed" toast and no way to enter the TOTP. The dispatcher (`tryShowTwoFactorPrompt`) is now called from all three connect paths (`connectToFtp`, `onSavedServerConnect`, `switchSession`) so the modal opens regardless of whether the user typed the credentials in Quick Connect, clicked a saved card, or switched back to a cached session tab. v3.6.8 was published with the modal feature flag in the CHANGELOG but the saved-card path silently bypassed it; the v3.6.9 cut on the same day fixes that hookup.
