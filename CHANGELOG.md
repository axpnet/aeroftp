...
- **MEGA 2FA TOTP support (issue #128)** — Quick Connect MEGA gains a "2FA Code" field below Password, mirroring the Filen / Internxt block (6-digit numeric, `inputMode="numeric"`, `autoComplete="one-time-code"`). Backend: `MegaConfig` extended with `two_factor_code: Option<String>` deserialized from `extra["two_factor_code"]`; both `login_v1` and `login_v2` inject `"mfa": <code>` into the `us` (login) request when present. The provider_commands TOTP forwarding branch in `provider_commands.rs` was extended from "Filen or Internxt" to also include MEGA, otherwise the frontend value was being silently dropped before it ever reached `MegaConfig::from_provider_config`. Server returns `-26` (E_MFAREQUIRED) when the field is absent on a 2FA-enabled account, and `-22` / `-16` on a wrong code; the dedicated error messages now surface to the user with actionable instructions instead of the generic "auth failed". `connect()` also clears any persisted `session_id` whenever a fresh TOTP is supplied, otherwise `try_resume_session()` would short-circuit with the pre-2FA session and the server-side `mfa` validation would never run. The MEGA API response parser also reads bodies as text first instead of going through reqwest's `.json()` helper, since MEGA returns global error codes (E_MFAREQUIRED, E_BLOCKED) with text/plain or no Content-Type at all, which made the helper bail with "error decoding response body" before the actual scalar `-26` could be classified. New `Save & Connect` button on the Edit Server form for MEGA / Filen / Internxt closes the UX loop: typing a fresh 6-digit code into the form and clicking Save & Connect now applies the code on a single connect attempt without re-saving the (single-use) TOTP into the profile. (reported by @EhudKirsh)
- **Two-Factor Authentication prompt modal** — when a connect attempt to MEGA / Filen / Internxt fails because the server is asking for a TOTP, AeroFTP now opens a small dedicated modal (provider logo + masked account label + 6-digit numeric input + "Open authenticator app" hint) instead of bouncing the user back to the saved-card grid with a generic "auth failed" toast. Submit retries the same connect with the code injected via `options.two_factor_code`; if the retry fails for the same reason (e.g. the user typed the wrong code or it had already rolled over), the modal reopens with the latest server message inline. The dispatcher (`tryShowTwoFactorPrompt`) is wired into all three connect paths — Quick Connect (`connectToFtp`), saved-card click (`onSavedServerConnect`) and tab reconnect (`switchSession`) — so the modal opens regardless of how the user initiated the connection. Esc and click-outside cancel. Five new i18n keys translated to all 47 languages.
- **AeroFTP master password TOTP setup QR code** — Settings > Security > Two-Factor Authentication > Setup now renders the actual QR code (`qrcode.react` `QRCodeSVG`, level M, 180px on a white tile so it scans cleanly under the Cyber / Tokyo Night themes) instead of just the otpauth URI as copyable text, so the user can point Authy / Google Authenticator / 1Password / Bitwarden directly at the dialog. Account name in the URI changed from "AeroFTP Vault" to "Desktop 2FA" — Authy was rendering the entry as the awkward "AeroFTP : AeroFTP Vault" duplication. The URI also now carries a Google-extension `image=https://docs.aeroftp.app/web-app-manifest-512x512.png` parameter so authenticators that honor it (FreeOTP+, Yubico Authenticator, Bitwarden, recent Google Authenticator) can show the AeroFTP logo. Authy ignores `image=` — it pulls icons from a Twilio-internal database, so an AeroFTP logo there would require a separate submission to Twilio support; until then Authy will fall back to its generic icon, which is a vendor limitation, not a URI problem.
