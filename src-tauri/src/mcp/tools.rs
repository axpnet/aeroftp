...
fn finish(tool: &str, server: Option<&str>, path: Option<&str>, result: (Value, bool), start: Instant) -> (Value, bool) {
...
pub async fn execute_tool(
    ...
    if let Err(retry_after) = rate_limiter.check(category) {
        ...
        return err(format!("Rate limit exceeded. Retry after {:.1} seconds.", retry_after));
    }
    ...
    match tool_name {
        ...
        "aeroftp_list_files" => {
            let server = match get_str(args, "server") { Ok(s) => s, Err(e) => return finish(tool_name, None, None, err(e), start) };
            let path = get_str_opt(args, "path").unwrap_or_else(|| "/".to_string());
            if let Err(e) = validate_sp(&server, Some(&path)) { return finish(tool_name, Some(&server), Some(&path), err(e), start); }
        ...
        "aeroftp_read_file" => {
            let server = match get_str(args, "server") { Ok(s) => s, Err(e) => return finish(tool_name, None, None, err(e), start) };
            let path = match get_str(args, "path") { Ok(s) => s, Err(e) => return finish(tool_name, Some(&server), None, err(e), start) };
            if let Err(e) = validate_sp(&server, Some(&path)) { return finish(tool_name, Some(&server), Some(&path), err(e), start); }
                        ...
                        Ok(entry) => match validate_read_preview_target(entry.is_dir, entry.size) {
                            ...
                            Ok(()) => match p.download_to_bytes(&path).await {
                                ...
                                Ok(data) => {
                                    ...
                                    ok(json!({ "path": path, "content": content, "size": data.len(), "truncated": truncated }))
                                }
                            }
                        }
                    }
        ...
        "aeroftp_search_files" => {
            let server = match get_str(args, "server") { Ok(s) => s, Err(e) => return finish(tool_name, None, None, err(e), start) };
            let pattern = match get_str(args, "pattern") { Ok(s) => s, Err(e) => return finish(tool_name, Some(&server), None, err(e), start) };
            let path = get_str_opt(args, "path").unwrap_or_else(|| "/".to_string());
            if let Err(e) = validate_sp(&server, Some(&path)) { return finish(tool_name, Some(&server), Some(&path), err(e), start); }
        ...
        "aeroftp_upload_file" => {
            let server = match get_str(args, "server") { Ok(s) => s, Err(e) => return finish(tool_name, None, None, err(e), start) };
            let remote_path = match get_str(args, "remote_path") { Ok(s) => s, Err(e) => return finish(tool_name, Some(&server), None, err(e), start) };
            if let Err(e) = validate_sp(&server, Some(&remote_path)) { return finish(tool_name, Some(&server), Some(&remote_path), err(e), start); }
            let local_path = get_str_opt(args, "local_path");
            let content = get_str_opt(args, "content");
            if local_path.is_none() && content.is_none() {
                return finish(tool_name, Some(&server), Some(&remote_path), err("Provide either 'local_path' or 'content'".into()), start);
            }
                ...
                Ok(arc) => {
                    ...
                    if let Some(ref local) = local_path {
                        ...
                        match p.upload(local, &remote_path, None).await {
                            Ok(()) => ok(json!({ "remote_path": remote_path, "uploaded": true, "bytes": bytes })),
                            Err(e) => err(sanitize_error(e)),
                        }
                    } else {
                        ...
                        if let Err(e) = std::fs::write(&tmp, text) {
                            return finish(tool_name, Some(&server), Some(&remote_path), err(sanitize_error(e)), start);
                        }
                        let upload_result = p.upload(tmp.to_str().unwrap_or(""), &remote_path, None).await;
        ...
        "aeroftp_download_file" => {
            let server = match get_str(args, "server") { Ok(s) => s, Err(e) => return finish(tool_name, None, None, err(e), start) };
            let remote_path = match get_str(args, "remote_path") { Ok(s) => s, Err(e) => return finish(tool_name, Some(&server), None, err(e), start) };
            let local_path = match get_str(args, "local_path") { Ok(s) => s, Err(e) => return finish(tool_name, Some(&server), Some(&remote_path), err(e), start) };
            if let Err(e) = validate_sp(&server, Some(&remote_path)) { return finish(tool_name, Some(&server), Some(&remote_path), err(e), start); }
            if let Err(e) = security::validate_local_path(&local_path) { return finish(tool_name, Some(&server), Some(&remote_path), err(e), start); }
            ...
            let result = match pool.get_provider(&server).await {
                ...
                Ok(arc) => {
                    ...
                    match p.download(&remote_path, &local_path, None).await {
                        Ok(()) => ok(json!({ "remote_path": remote_path, "local_path": local_path, "downloaded": true })),
                        Err(e) => err(sanitize_error(e)),
                    }
                }
            };
                ...
                Ok(arc) => {
                    ...
                    match p.stat(&path).await {
                        ...
                        Ok(entry) => match delete_kind(entry.is_dir, recursive) {
                            ...
                            DeleteKind::DirectoryRecursive => match p.rmdir_recursive(&path).await {
                                Ok(()) => ok(json!({
                                    "path": path,
                                    "deleted": true,
                                    "is_dir": true,
        ...
        "aeroftp_rename" => {
            let server = match get_str(args, "server") { Ok(s) => s, Err(e) => return finish(tool_name, None, None, err(e), start) };
            let from = match get_str(args, "from") { Ok(s) => s, Err(e) => return finish(tool_name, Some(&server), None, err(e), start) };
            let to = match get_str(args, "to") { Ok(s) => s, Err(e) => return finish(tool_name, Some(&server), Some(&from), err(e), start) };
            if let Err(e) = validate_sp(&server, Some(&from)) { return finish(tool_name, Some(&server), Some(&from), err(e), start); }
            if let Err(e) = security::validate_remote_path(&to) { return finish(tool_name, Some(&server), Some(&from), err(e), start); }
        ...
        "aeroftp_storage_quota" => {
            let server = match get_str(args, "server") { Ok(s) => s, Err(e) => return finish(tool_name, None, None, err(e), start) };
            if let Err(e) = security::validate_server_query(&server) { return finish(tool_name, Some(&server), None, err(e), start); }
                ...
                Ok(arc) => {
                    ...
                    match p.storage_info().await {
                        ...
                        Ok(info) => {
                            ...
                            ok(json!({ "used": info.used, "total": info.total, "free": info.free, "used_pct": used_pct }))
                        }
                    }
                }
            };
        ...
        "aeroftp_create_share_link" => {
            ...
            let result = match pool.get_provider(&server).await {
                ...
                Ok(arc) => {
                    let mut p = arc.lock().await;
                    let opts = ShareLinkOptions { expires_in_secs: expires, password, permissions: None };
                    match p.create_share_link(&path, opts).await {
                        Err(e) => err(sanitize_error(e)),
                        Ok(link) => ok(json!({ "url": link.url, "password": link.password, "expires_at": link.expires_at })),
                    }
                }
            };
            finish(tool_name, Some(&server), Some(&path), result, start)
            ...
            let result = match pool.get_provider(&server).await {
                ...
                Ok(arc) => {
                    ...
                    match p.list_versions(&path).await {
                        Err(e) => err(sanitize_error(e)),
                        Ok(versions) => {
                            let items: Vec<Value> = versions.iter().map(|v| json!({
...
#[cfg(test)]
mod tests {
    use super::{delete_kind, tool_definitions, validate_read_preview_target, DeleteKind, MAX_READ_PREVIEW_BYTES};
    ...
    fn delete_tool_schema_exposes_recursive_flag() {
        ...
        let recursive = delete_tool.input_schema
            .get("properties")
