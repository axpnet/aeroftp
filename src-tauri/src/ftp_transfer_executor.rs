        ...
        for attempt in 0..=retry_policy.max_retries {
            ...
            let result = match self.pool.acquire().await {
                Ok(lease) => {
                    let transfer_result = {
                        ...
                        async move {
                            ...
                            ftp.change_dir(&parent_dir)
                                .await
                                .map_err(|e| format!("Failed to change directory to {}: {}", parent_dir, e))?;

                            let started_at = Instant::now();
                            ftp.download_file_with_progress(&remote_name, &local_path, |transferred| {
                            ...
                            crate::preserve_remote_mtime(&local_path_for_mtime, modified.as_deref());
                            Ok::<(), String>(())
                        }
                    }
                    .await;
                    ...
                    if let Err(release_error) = lease.release().await {
                        warn!("Failed to reset FTP lease after {}: {}", entry.remote_path, release_error);
                    }
            ...
            match result {
                ...
                Err(error) => {
                    ...
                    if self.cancel_flag.load(Ordering::Relaxed)
                        || last_error.contains("cancelled by user")
                    {
                        break;
                    }
...
fn split_remote_path(remote_path: &str) -> (String, String) {
    ...
    let normalized_parent = if parent.is_empty() { "/".to_string() } else { parent };
