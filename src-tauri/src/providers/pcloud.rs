...
impl StorageProvider for PCloudProvider {
    ...
    async fn connect(&mut self) -> Result<(), ProviderError> {
        ...
        if !resp.status().is_success() {
            return Err(ProviderError::AuthenticationFailed(
                "pCloud authentication failed".to_string(),
            ));
        }
        ...
        if let Some(result) = body["result"].as_i64() {
            if result != 0 {
                ...
                return Err(ProviderError::AuthenticationFailed(sanitize_api_error(
                    error_msg,
                )));
            }
        }
    ...
    async fn upload(
        ...
        if upload_resp.result != 0 {
            let msg = upload_resp
                .error
                .unwrap_or_else(|| format!("Upload error code: {}", upload_resp.result));
    ...
    async fn stat(&mut self, path: &str) -> Result<RemoteEntry, ProviderError> {
        ...
        if resp.result != 0 {
            ...
            let folder_resp: PCloudResponse = self
                .get_with_retry(&url, &auth)
                .await?
                .json()
                .await
    ...
    async fn create_share_link(
        ...
        if let Some(secs) = options.expires_in_secs {
            ...
            url.push_str(&format!(
                "&expire={}",
                urlencoding::encode(&expires_at.format("%Y-%m-%dT%H:%M:%S+0000").to_string())
            ));
        }
        ...
        if resp.result != 0 {
            return Err(ProviderError::Other(sanitize_api_error(
                &resp
                    .error
                    .unwrap_or_else(|| "Failed to create link".to_string()),
            )));
        }

        let link_url = resp
            .link
            .ok_or_else(|| ProviderError::Other("No link in response".to_string()))?;
    ...
    async fn remove_share_link(&mut self, path: &str) -> Result<(), ProviderError> {
        ...
        let link_id = links_resp
            ...
            .ok_or_else(|| {
                ProviderError::NotFound(format!("No public link found for path: {}", resolved))
            })?;
    ...
    async fn get_thumbnail(&mut self, path: &str) -> Result<String, ProviderError> {
        ...
        if resp.result != 0 {
            return Err(ProviderError::NotFound(sanitize_api_error(
                &resp
                    .error
                    .unwrap_or_else(|| "No thumbnail available".to_string()),
            )));
        }
    ...
    async fn list_versions(&mut self, path: &str) -> Result<Vec<FileVersion>, ProviderError> {
        ...
        if resp.result != 0 {
            return Err(ProviderError::Other(sanitize_api_error(
                &resp
                    .error
                    .unwrap_or_else(|| "Failed to list revisions".to_string()),
            )));
        }
    ...
    async fn download_version(
        ...
        if link_resp.result != 0 {
            return Err(ProviderError::TransferFailed(sanitize_api_error(
                &link_resp
                    .error
                    .unwrap_or_else(|| "Failed to get version link".to_string()),
            )));
        }
    ...
    async fn checksum(&mut self, path: &str) -> Result<HashMap<String, String>, ProviderError> {
        ...
        let auth = self.auth_header().await?;
        let resp: ChecksumResponse = self
            .get_with_retry(&url, &auth)
            .await?
            .json()
            .await
        ...
        if let Some(v) = resp.sha256 {
            checksums.insert("sha256".to_string(), v);
        }
        if let Some(v) = resp.sha1 {
            checksums.insert("sha1".to_string(), v);
        }
        if let Some(v) = resp.md5 {
            checksums.insert("md5".to_string(), v);
        }
        Ok(checksums)
    }
...
impl PCloudProvider {
    ...
    pub async fn list_trash(&self) -> Result<Vec<RemoteEntry>, ProviderError> {
        ...
        let metadata = resp.metadata.ok_or_else(|| {
            ProviderError::ParseError("No metadata in trash response".to_string())
        })?;

        let contents = metadata.contents.unwrap_or_default();
        let entries = contents
            .iter()
            .filter(|m| !m.isfolder || m.fileid.is_some() || m.folderid.is_some())
            .map(|m| {
