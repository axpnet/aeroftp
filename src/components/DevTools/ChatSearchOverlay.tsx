...
/**
 * Utility: Highlight search matches in a text string.
 * Returns HTML with <mark> tags for highlighting.
 * IMPORTANT: Input text must already be HTML-escaped before calling this.
 */
export function highlightSearchMatches(
    ...
    const messageMatches = allMatches.filter(m => m.messageId === messageId);
    if (messageMatches.length === 0) return text;
    ...
    while ((pos = lowerText.indexOf(lowerQuery, pos)) !== -1) {
        ...
        const isActive = activeMatch !== null &&
            ...
            activeMatch.startOffset === pos;

        const matchText = text.slice(pos, pos + query.length);
        // Defensive HTML escaping to prevent XSS even if input was not pre-escaped
        const safeMatch = matchText.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
        const cls = isActive
            ? 'bg-yellow-400 text-black rounded-sm'
            : 'bg-yellow-600/40 text-yellow-200 rounded-sm';
        parts.push(`<mark class="${cls}">${safeMatch}</mark>`);
