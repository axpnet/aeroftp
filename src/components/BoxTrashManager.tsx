...
interface BoxTrashManagerProps {
  onClose: () => void;
  onRefreshFiles?: () => void;
}
...
export function BoxTrashManager({ onClose, onRefreshFiles }: BoxTrashManagerProps) {
  ...
  const loadTrash = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await invoke<TrashEntry[]>('box_list_trash');
      setItems(result);
      setSelected(new Set());
    } catch (err) {
      setError(String(err));
  ...
  const getItemType = (item: TrashEntry): string => item.metadata?.item_type || (item.is_dir ? 'folder' : 'file');
                  ...
                  return (
                    <tr
                      ...
                      className={`cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 border-b border-gray-200 dark:border-gray-700/30 ${
                        selected.has(itemId) ? 'bg-blue-500/10' : ''
                      }`}
