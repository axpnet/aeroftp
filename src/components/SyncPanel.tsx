...
async function getJournalSigningKey(
  localPath: string,
  remotePath: string,
): Promise<string> {
...
interface DeltaEligibilityPromptState {
  reason: string | null;
  serverIdentity: DeltaServerIdentity | null;
  canPersistPreference: boolean;
}

const SAVED_SERVERS_STORAGE_KEY = "aeroftp-saved-servers";
const DELTA_SYNC_DOCS_URL = "https://docs.aeroftp.app/features/delta-sync";

const matchesServerProfileIdentity = (
  profile: ServerProfile,
  identity: DeltaServerIdentity,
): boolean => {
  return (
    (profile.protocol || "ftp") === identity.protocol &&
...
export const SyncPanel: React.FC<SyncPanelProps> = ({
  ...
  const [deltaEligibilityPrompt, setDeltaEligibilityPrompt] =
    useState<DeltaEligibilityPromptState | null>(null);
  const [skipDeltaEligibilityPrompt, setSkipDeltaEligibilityPrompt] =
    useState(false);

  // --- Rename Tracking ---
  const [detectRenames, setDetectRenames] = useState(false);
  const [detectedRenames, setDetectedRenames] = useState<Map<string, string>>(
    new Map(),
  ); // new_path → old_path
  ...
  const [fileResults, setFileResults] = useState<Map<string, FileSyncResult>>(
    new Map(),
  );
  ...
  const fallbackReasonRef = useRef<Map<string, string>>(new Map());
  const [fallbackReasonVersion, setFallbackReasonVersion] = useState(0);
  const [hasIndex, setHasIndex] = useState(false);
  const [hasJournal, setHasJournal] = useState(false);
  const [pendingJournal, setPendingJournal] = useState<SyncJournal | null>(
    null,
  );
  ...
  const [journalIntegrity, setJournalIntegrity] = useState<
    Map<string, JournalIntegrityStatus>
  >(new Map());
  ...
  const {
    ...
    activeProfileId,
    setActiveProfileId,
    applyProfile: getProfileResult,
  } = useSyncProfiles(isOpen);

  // Provider optimization hints
  const { hints: optimizationHints } = useSyncOptimization(
    protocol,
    isConnected,
  );
  ...
  useEffect(() => {
    if (
      optimizationHints &&
      !optimizationHints.delta_sync_active &&
      deltaSyncEnabled
    ) {
  ...
  const progressFlushTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const getSavedServerProfile = useCallback(
    async (identity: DeltaServerIdentity): Promise<ServerProfile | null> => {
      const savedServers = await secureGetWithFallback<ServerProfile[]>(
        "server_profiles",
        SAVED_SERVERS_STORAGE_KEY,
      );
      if (!savedServers?.length) return null;
      return (
        savedServers.find((profile) =>
          matchesServerProfileIdentity(profile, identity),
        ) || null
      );
    },
    [],
  ...
  const persistDeltaEligibilityPreference = useCallback(
    async (identity: DeltaServerIdentity) => {
      const savedServers = await secureGetWithFallback<ServerProfile[]>(
        "server_profiles",
        SAVED_SERVERS_STORAGE_KEY,
      );
      if (!savedServers?.length) return;

      let updated = false;
      const nextServers = savedServers.map((profile) => {
        if (
          !matchesServerProfileIdentity(profile, identity) ||
          profile.skipDeltaEligibilityPrompt
        ) {
          return profile;
        }
        updated = true;
  ...
  const resolveDeltaEligibilityPrompt = useCallback(
    async (proceed: boolean) => {
      const promptState = deltaEligibilityPrompt;
      const resolver = deltaEligibilityResolverRef.current;
      deltaEligibilityResolverRef.current = null;
      setDeltaEligibilityPrompt(null);

      if (
        proceed &&
        skipDeltaEligibilityPrompt &&
        promptState?.canPersistPreference &&
        promptState.serverIdentity
      ) {
        try {
          await persistDeltaEligibilityPreference(promptState.serverIdentity);
        } catch {
          /* ignore */
        }
      }

      setSkipDeltaEligibilityPrompt(false);
      resolver?.(proceed);
    },
    [
      deltaEligibilityPrompt,
      persistDeltaEligibilityPreference,
      skipDeltaEligibilityPrompt,
    ],
  );

  const maybeConfirmDeltaEligibility =
    useCallback(async (): Promise<boolean> => {
      if (protocol !== "sftp" || !deltaSyncEnabled) {
        return true;
      }

      let probe: DeltaEligibilityProbeResult;
      try {
        probe = await invoke<DeltaEligibilityProbeResult>(
          "sftp_probe_delta_eligibility",
        );
      } catch (probeError) {
        console.warn(
          "[SyncPanel] sftp_probe_delta_eligibility failed:",
          probeError,
        );
        return true;
      }
      ...
      let canPersistPreference = false;
      if (probe.server_identity) {
        const savedProfile = await getSavedServerProfile(probe.server_identity);
        if (savedProfile?.skipDeltaEligibilityPrompt) {
          return true;
        }
        canPersistPreference = !!savedProfile;
      }

      return await new Promise<boolean>((resolve) => {
        deltaEligibilityResolverRef.current = resolve;
        setSkipDeltaEligibilityPrompt(false);
        setDeltaEligibilityPrompt({
          reason: probe.reason,
          serverIdentity: probe.server_identity,
      ...
      invoke<JournalSummary[]>("list_sync_journals_cmd")
        .then(async (journals) => {
          ...
          for (const j of journals) {
            ...
            try {
              const signingKey = await getJournalSigningKey(
                j.local_path,
                j.remote_path,
              );
  ...
  useEffect(() => {
    const loadDefaults = async () => {
      try {
        const defaults = await invoke<CompareOptions>(
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      const existingIndex = await invoke<SyncIndex | null>(
        "load_sync_index_cmd",
        {
      ...
      const command = isProvider
        ? "provider_compare_directories"
        : "compare_directories";
      ...
      const rawResultCount = results.filter(
      ...
      if (detectRenames) {
        try {
          const renames = await invoke<
            Array<{ old_path: string; new_path: string; size: number }>
          >("detect_renames_cmd", {
            localPath: editLocalPath,
          ...
          if (removePaths.size > 0) {
            differences = differences.filter(
              (d) => !removePaths.has(d.relative_path),
            );
          }
        } catch {
      ...
      if (strategy !== "ask") {
        ...
        for (const c of differences) {
          ...
          if (strategy === "newer") {
            const lt = c.local_info?.modified
              ? new Date(c.local_info.modified).getTime()
              : 0;
            const rt = c.remote_info?.modified
              ? new Date(c.remote_info.modified).getTime()
              : 0;
            autoResolutions.set(
              c.relative_path,
              lt >= rt ? "upload" : "download",
            );
            ...
            autoResolutions.set(
              c.relative_path,
              (c.local_info?.size || 0) >= (c.remote_info?.size || 0)
                ? "upload"
                : "download",
            );
  ...
  const syncableDirs = comparisons.filter(
    (c) =>
      c.is_dir &&
      ((c.status === "remote_only" &&
        (options.direction === "remote_to_local" ||
          options.direction === "bidirectional")) ||
        (c.status === "local_only" &&
          (options.direction === "local_to_remote" ||
            options.direction === "bidirectional"))),
  ).length;
  ...
  const isDeleteOrphan = (
    status: SyncStatus,
    comparison?: FileComparison,
  ): boolean => {
    if (!options.delete_orphans) return false;
    if (options.direction === "local_to_remote" && status === "remote_only")
      return true;
    if (options.direction === "remote_to_local" && status === "local_only")
      return true;
    if (
      options.direction === "bidirectional" &&
  ...
  const deleteCount = comparisons.filter(
    (c) =>
      !c.is_dir &&
      selectedPaths.has(c.relative_path) &&
      isDeleteOrphan(c.status, c),
  ).length;
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      if (filePath) {
        const header =
          "path,status,is_dir,sync_reason,local_size,local_modified,remote_size,remote_modified";
        const escape = (s: string) => `"${(s || "").replace(/"/g, '""')}"`;
        const rows = comparisons.map((c) =>
          [
            escape(c.relative_path),
            c.status,
            c.is_dir,
            escape(c.sync_reason),
            c.local_info?.size ?? "",
            c.local_info?.modified ?? "",
            c.remote_info?.size ?? "",
            c.remote_info?.modified ?? "",
          ].join(","),
        );
  ...
  const safetyScore = React.useMemo(() => {
    ...
    const conflicts = files.filter(
      (c) => c.status === "conflict" || c.status === "size_mismatch",
    ).length;
    const dangerousExts = [
      ".exe",
      ".sh",
    ...
    const dangerous = files.filter((c) =>
      dangerousExts.some((ext) => c.relative_path.toLowerCase().endsWith(ext)),
    ).length;
    const totalSize = files.reduce(
      (sum, c) => sum + (c.local_info?.size ?? 0) + (c.remote_info?.size ?? 0),
      0,
    );
    const level: "low" | "medium" | "high" =
      conflicts > 5 || dangerous > 10
        ? "high"
        : conflicts > 0 || dangerous > 0
          ? "medium"
          : "low";
  ...
  const handleVerifyJournal = async (lp: string, rp: string) => {
    ...
    try {
      ...
      setJournalIntegrity((prev) =>
        new Map(prev).set(key, valid ? "verified" : "tampered"),
      );
      setJournalFeedback(
        valid
          ? t("syncPanel.journalVerifySuccess")
          : t("syncPanel.journalVerifyFailed"),
      );
  ...
  const applyBandwidthSchedule = async () => {
    ...
    const shouldThrottle =
      (schedule === "office" && isOfficeHours) ||
      (schedule === "night" && !isOfficeHours);
  ...
  const unresolvedConflicts = conflicts.filter(
    (c) => !conflictResolutions.has(c.relative_path),
  );
  ...
  const resolveAllKeepNewer = () => {
    ...
    const sel = new Set(selectedPaths);
    for (const c of conflicts) {
      const localTime = c.local_info?.modified
        ? new Date(c.local_info.modified).getTime()
        : 0;
      const remoteTime = c.remote_info?.modified
        ? new Date(c.remote_info.modified).getTime()
        : 0;
      const resolution: ConflictResolution =
        localTime >= remoteTime ? "upload" : "download";
      updates.set(c.relative_path, resolution);
      sel.add(c.relative_path);
    }
    setConflictResolutions(updates);
  ...
  const getRetryDelay = (attempt: number): number => {
    const d =
      ...
      Math.pow(retryPolicy.backoff_multiplier, attempt - 1);
  ...
  const executeTransferWithRetry = async (
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const errorInfo = await invoke<SyncErrorInfo>(
  ...
  const handleSync = async (resumeJournal?: SyncJournal) => {
    ...
    const canProceedWithSync = await maybeConfirmDeltaEligibility();
    ...
    if (resumeJournal && comparisons.length === 0) {
      ...
      selectedComparisons = resumeJournal.entries.map((e) => ({
        ...
        status: (e.action === "upload"
          ? "local_newer"
          : "remote_newer") as SyncStatus,
        is_dir: false,
        local_info:
          e.action === "upload"
            ? {
        ...
        sync_reason:
          e.action === "upload"
            ? "Resumed from journal (upload)"
            : "Resumed from journal (download)",
      }));
    } else {
      selectedComparisons = comparisons.filter(
        (c) => selectedPaths.has(c.relative_path) && !c.is_dir,
      );
    }
    ...
    if (resumeJournal) {
      ...
      for (const e of resumeJournal.entries) {
        if (e.status === "completed")
          initialResults.set(e.relative_path, "success");
        else if (e.status === "skipped")
          initialResults.set(e.relative_path, "skipped");
      }
    }
    ...
    fallbackReasonRef.current = new Map();
    setFallbackReasonVersion(0);
    ...
    // the swap is a no-op.
    const captureTransferMetadataForPath = (relativePath: string) => {
      const basename = relativePath.split("/").pop() || relativePath;
      if (basename !== relativePath) {
        const deltaStats = deltaStatsRef.current.get(basename);
        if (deltaStats) {
          deltaStatsRef.current.delete(basename);
          deltaStatsRef.current.set(relativePath, deltaStats);
          setDeltaStatsVersion((v) => v + 1);
        }
        const fallbackReason = fallbackReasonRef.current.get(basename);
        if (fallbackReason) {
          fallbackReasonRef.current.delete(basename);
          fallbackReasonRef.current.set(relativePath, fallbackReason);
          setFallbackReasonVersion((v) => v + 1);
        }
      }
    };
    ...
    const onTransferEvent = (event: Event) => {
      ...
      if (payload?.event_type === "complete" && payload.fallback_reason) {
        fallbackReasonRef.current.set(
          payload.filename,
    ...
    const journal: SyncJournal = resumeJournal || {
      ...
      entries: selectedComparisons.map((c) => {
        ...
        if (isDeleteOrphan(c.status, c)) {
          ...
          action =
            c.status === "local_newer" || c.status === "local_only"
              ? "upload"
              : "download";
        }
        return {
    ...
    if (
      selectedComparisons.some(
        (c) => c.status === "local_newer" || c.status === "local_only",
      )
    ) {
      const dirsToCreate = new Set<string>();
      for (const item of selectedComparisons) {
        const shouldUpload =
          (item.status === "local_newer" || item.status === "local_only") &&
          (options.direction === "local_to_remote" ||
            options.direction === "bidirectional");
    ...
    if (
      selectedComparisons.some(
        (c) => c.status === "remote_newer" || c.status === "remote_only",
      )
    ) {
      const dirsToCreate = new Set<string>();
      for (const item of selectedComparisons) {
        const shouldDownload =
          (item.status === "remote_newer" || item.status === "remote_only") &&
          (options.direction === "remote_to_local" ||
            options.direction === "bidirectional");
    ...
    let dirsCreated = 0;
    const dirComparisons = comparisons.filter(
      (c) =>
        c.is_dir &&
        ((c.status === "remote_only" &&
          (options.direction === "remote_to_local" ||
    ...
    for (const dir of dirComparisons.sort(
      (a, b) =>
        a.relative_path.split("/").length - b.relative_path.split("/").length,
    )) {
      try {
    ...
    for (let i = 0; i < selectedComparisons.length; i++) {
      ...
      const journalEntry =
        entryIdx !== undefined ? journal.entries[entryIdx] : undefined;

      // Skip already completed entries (for resume)
      if (
        resumeJournal &&
        (journalEntry?.status === "completed" ||
          journalEntry?.status === "skipped")
      ) {
      ...
      if (cancelledRef.current) {
        ...
        for (let j = i; j < selectedComparisons.length; j++) {
          const jIdx = journalEntryMap.get(
            selectedComparisons[j].relative_path,
          );
          if (
            jIdx !== undefined &&
            journal.entries[jIdx] &&
            journal.entries[jIdx].status === "pending"
          ) {
      ...
      const conflictRes =
        item.status === "conflict"
          ? conflictResolutions.get(item.relative_path)
          : undefined;
      ...
      if (shouldUpload) {
        ...
        const args = isProvider
          ? {
              ...
              useDelta: deltaSyncEnabled,
            }
          : {
              params: {
                local_path: localFilePath,
                remote_path: remoteFilePath,
                use_delta: deltaSyncEnabled,
              },
            };
        ...
        if (result.success) {
          ...
          captureTransferMetadataForPath(item.relative_path);
        ...
        if (result.success) {
          ...
          const vResult = await verifyDownload(
            localFilePath,
            expectedSize,
            expectedMtime,
          );
        ...
        try {
          if (item.status === "remote_only") {
            ...
            if (!item.is_dir) {
              await archiveLocalBeforeMutation(
                localFilePath,
                item.relative_path,
              );
            }
      ...
      const checkpointInterval =
        selectedComparisons.length > 2000
          ? 200
          : selectedComparisons.length > 500
            ? 100
            : 10;
    ...
    try {
      ...
      if (deleted > 0) {
        for (const item of selectedComparisons) {
          if (
            isDeleteOrphan(item.status, item) &&
            fileResults.get(item.relative_path) === "success"
          ) {
  ...
  const getFileResultIcon = (path: string): React.ReactNode => {
    ...
    if (result === "retrying")
      return <RotateCcw size={14} className="animate-spin text-amber-400" />;
    if (result === "verifying")
      return <ShieldCheck size={14} className="animate-pulse text-cyan-400" />;
    if (result === "success")
  ...
  const visibleRowCount =
    Math.ceil(VIRTUAL_VIEWPORT / VIRTUAL_ROW_HEIGHT) + VIRTUAL_OVERSCAN * 2;
    ...
    >
      <div className="sync-panel">
        <div className="sync-panel-header">
          <div className="flex items-center gap-3">
            <h2>
              <FolderSync size={20} className="inline mr-2" />{" "}
              {t("syncPanel.title")}
            </h2>
            {speedMode !== "normal" && (
              <span
                className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${speedMode === "maniac" ? "bg-red-500/20 text-red-400" : speedMode === "extreme" ? "bg-orange-500/20 text-orange-400" : speedMode === "turbo" ? "bg-yellow-500/20 text-yellow-400" : "bg-blue-500/20 text-blue-400"}`}
              >
          ...
          <div className="mx-4 my-2 p-3 rounded-lg border bg-amber-50 dark:bg-amber-900/20 border-amber-300 dark:border-amber-700">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-semibold text-sm">
                {t("syncPanel.journalFound")}
              </span>
            </div>
            <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">
              {t("syncPanel.journalDescription", {
                completed: String(
                  pendingJournal.entries.filter((e) => e.status === "completed")
                    .length,
                ),
            ...
            {showJournalHistory && (
              <div className="mt-1 space-y-1 max-h-48 overflow-y-auto pr-1">
                  ...
                  <div
                    ...
                    style={{
                      background:
                        journalFeedback ===
                          t("syncPanel.journalVerifySuccess") ||
                        journalFeedback === t("syncPanel.journalSignSuccess")
                          ? "rgba(16,185,129,0.1)"
                          : "rgba(239,68,68,0.1)",
                      borderColor:
                        journalFeedback ===
                ...
                {journalList.map((j) => {
                  ...
                  const isJournalVerified =
                    intStatus === "verified" || intStatus === "signed";
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background:
                          "var(--color-bg-secondary, rgba(0,0,0,0.05))",
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        >
                          ...
                          <span
                            className={
                              j.completed ? "text-green-500" : "text-amber-500"
                            }
                          >
                            {j.completed
                              ? t("syncPanel.journalCompleted")
                              : t("syncPanel.journalInterrupted")}
                          </span>
                        </div>
                      </div>
                      <div className="flex-shrink-0">
                      ...
                      <div className="flex-shrink-0 flex gap-1">
                        {!isJournalVerified && !isJournalTampered && (
                          <button
                            ...
                            onClick={() =>
                              handleSignJournal(j.local_path, j.remote_path)
                            }
                        ...
                        {(isJournalVerified || isJournalTampered) && (
                          <button
                            ...
                            onClick={() =>
                              handleVerifyJournal(j.local_path, j.remote_path)
                            }
                            disabled={isJournalChecking}
              ...
              <div className="mt-2 max-h-48 overflow-y-auto space-y-1">
                {conflicts.map((c) => {
                  ...
                  return (
                    <div
                      key={c.relative_path}
                      className="flex items-center gap-2 text-xs py-1 border-b border-gray-200/10 last:border-0"
                    >
                      <File size={12} className="text-gray-400 flex-shrink-0" />
                      <span
                        className="flex-1 truncate text-gray-300"
                      ...
                      <div className="flex gap-1 flex-shrink-0">
                        <button
                          ...
                          onClick={() =>
                            resolveConflict(c.relative_path, "upload")
                          }
                        ...
                        <button
                          ...
                          onClick={() =>
                            resolveConflict(c.relative_path, "download")
                          }
                        ...
                        <button
                          ...
                          onClick={() =>
                            resolveConflict(c.relative_path, "skip")
                          }
        ...
        {syncReport && (
          <div
            className={`mx-4 my-3 p-4 rounded-lg border ${syncReport.errors.length > 0 ? "bg-yellow-50 dark:bg-yellow-900/20 border-yellow-300 dark:border-yellow-700" : "bg-green-50 dark:bg-green-900/20 border-green-300 dark:border-green-700"}`}
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              {syncReport.dirsCreated > 0 && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    {t("syncPanel.reportDirsCreated")}:{" "}
                    <strong>{syncReport.dirsCreated}</strong>
                  </span>
                </div>
              )}
              ...
              {syncReport.retried > 0 && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    {t("syncPanel.reportRetried")}:{" "}
                    <strong>{syncReport.retried}</strong>
                  </span>
                </div>
              )}
              {syncReport.verifyFailed > 0 && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    {t("syncPanel.reportVerifyFailed")}:{" "}
                    <strong>{syncReport.verifyFailed}</strong>
                  </span>
                </div>
              )}
              {syncReport.errors.length > 0 && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    {t("syncPanel.reportErrors")}:{" "}
                    <strong>{syncReport.errors.length}</strong>
                  </span>
                </div>
              )}
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  ...
                  <strong>{formatSize(syncReport.totalBytes)}</strong>
                </span>
              </div>
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  ...
                  <strong>{formatDuration(syncReport.durationMs)}</strong>
                </span>
              </div>
            </div>
            ...
            {syncReport.errors.length > 0 && (
              <div className="mt-3 border-t border-gray-200 dark:border-gray-600 pt-2">
                ...
                {[...groupErrorsByKind(syncReport.errors)].map(
                  ([kind, errs]) => {
                ...
                onOptionsChange={(newOpts) => {
                  ...
                  if (newOpts.direction !== options.direction) {
                    const match = profiles.find(
                      (p) => p.builtin && p.direction === newOpts.direction,
                    );
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              {hasIndex && (
                <span
                  ...
                  title={t("syncPanel.indexAvailableTooltip")}
                >
            ...
            <div className="sync-empty">
              {!isConnected
                ...
                : lastCompare === null
                  ...
                  : lastCompare.rawResultCount === 0
                    ? t("syncPanel.allInSync")
                    : t("syncPanel.nothingInDirection")}
            </div>
          )}
              ...
              <div className="sync-table-header">
                <div className="sync-col-check">
                  <Checkbox
                    checked={
                      selectedPaths.size ===
                        comparisons.filter((c) => !c.is_dir).length &&
                      comparisons.length > 0
                    }
                    onChange={() =>
                      selectedPaths.size ===
                ...
                <div className="sync-col-status">
                  {t("syncPanel.colStatus")}
                </div>
                ...
                >
                  ...
                  >
                    {comparisons
                      ...
                      .map((comparison) => {
                        if (!comparison || !comparison.relative_path)
                          return null;
                        const isOrphanDelete = isDeleteOrphan(
                          comparison.status,
                          comparison,
                        );
                        ...
                        const resultIcon = getFileResultIcon(
                          comparison.relative_path,
                        );
                        const result = fileResults.get(
                          comparison.relative_path,
                        );
                        ...
                        void fallbackReasonVersion;
                        const deltaEntry = deltaStatsRef.current.get(
                          comparison.relative_path,
                        );
                        const fallbackReason = fallbackReasonRef.current.get(
                          comparison.relative_path,
                        );
                        return (
                          <div
                            ...
                            onClick={() =>
                              !isSyncing &&
                              !comparison.is_dir &&
                              toggleSelection(comparison.relative_path)
                            }
                            style={{
                          ...
                          >
                            ...
                            <div
                              className="sync-col-status"
                              style={{ color: statusCfg.color }}
                              title={comparison.sync_reason}
                            >
                              <StatusIcon size={14} />
                              <span className="status-label">
                                {getStatusLabel(comparison.status, comparison)}
                              </span>
                            </div>
                            <div className="sync-col-file">
                              <div className="flex flex-col min-w-0">
                                ...
                                {comparison.sync_reason &&
                                  comparison.status !== "identical" && (
                                    <span className="sync-reason-text">
                                      {comparison.sync_reason}
                                    </span>
                                  )}
                              </div>
                            ...
                            <div className="sync-col-result flex items-center gap-1">
                              ...
                              {!deltaEntry && fallbackReason && (
                                <span
                                  className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600"
                                  title={t("syncPanel.classicBadgeTooltip", {
                                    reason: fallbackReason,
                                  })}
                                >
                                  <Info size={9} />
                                  {t("syncPanel.classicBadge")}
                                </span>
                              )}
                            </div>
          ...
          <div className="sync-summary">
            {syncProgress ? (
              <span className="sync-progress-indicator">
                ...
                {t("syncPanel.syncing")} {syncProgress.current}/
                {syncProgress.total}...
              </span>
            ) : syncReport ? (
            ...
            {safetyScore && !isSyncing && !syncReport && (
              <span
                ...
                title={[
                  ...
                  .join(" · ")}
              >
                {safetyScore.level === "high" ? (
                  <ShieldAlert size={14} />
                ) : (
                  <ShieldCheck size={14} />
                )}
                {safetyScore.level === "low"
                  ? t("syncPanel.safetyLow")
                  : safetyScore.level === "medium"
                    ? t("syncPanel.safetyMedium")
          ...
          <div className="sync-actions">
            {isSyncing ? (
              ...
              <>
                <button
                  onClick={handleReset}
                  disabled={comparisons.length === 0 && !syncReport}
                >
            ...
            {syncReport ? (
              ...
              >
                {isSyncing ? (
                  <>
                    <Loader2 size={16} className="animate-spin" />{" "}
                    {t("syncPanel.syncing")} ({syncProgress?.current || 0}/
                    {syncProgress?.total || 0})...
                  </>
                ) : canaryMode ? (
                  <>
                    ...
                    {selectedPaths.size + syncableDirs})
                  </>
                ) : (
                  <>
                    <Zap size={16} /> {t("syncPanel.synchronize")} (
                    {selectedPaths.size + syncableDirs})
                  </>
                )}
              </button>
      ...
      {deltaEligibilityPrompt && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={t("syncPanel.deltaEligibilityTitle")}
          onClick={(event) => {
            if (event.target === event.currentTarget) {
              void resolveDeltaEligibilityPrompt(false);
            }
          }}
        >
          <div className="w-full max-w-lg rounded-xl border border-amber-500/30 bg-white dark:bg-gray-800 shadow-2xl overflow-hidden">
            <div className="flex items-start gap-3 p-5">
              <div className="mt-0.5 flex-shrink-0 rounded-full bg-amber-500/15 p-2 text-amber-400">
                <AlertTriangle size={18} />
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">
                  {t("syncPanel.deltaEligibilityTitle")}
                </h3>
                <p className="mt-2 text-sm leading-relaxed text-gray-600 dark:text-gray-300">
                  {t("syncPanel.deltaEligibilityBody")}
                </p>
                {deltaEligibilityPrompt.reason && (
                  <p className="mt-2 rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs leading-relaxed text-amber-700 dark:text-amber-200">
                    {deltaEligibilityPrompt.reason}
                  </p>
                )}
                {deltaEligibilityPrompt.canPersistPreference && (
                  <div className="mt-4">
                    <Checkbox
                      checked={skipDeltaEligibilityPrompt}
                      onChange={setSkipDeltaEligibilityPrompt}
                      label={t("syncPanel.deltaEligibilitySkip")}
                    />
                  </div>
                )}
              </div>
            </div>
            <div className="flex flex-wrap items-center justify-between gap-2 border-t border-gray-200 bg-gray-50 px-5 py-4 dark:border-gray-700 dark:bg-gray-800/80">
              <button
                ...
                onClick={() => openUrl(DELTA_SYNC_DOCS_URL)}
              >
                {t("syncPanel.deltaEligibilityLearnMore")}
              </button>
              <div className="flex items-center gap-2">
                <button
                ...
                <button
                  ...
                  onClick={() => {
                    void resolveDeltaEligibilityPrompt(true);
                  }}
                >
                  {t("syncPanel.deltaEligibilityProceed")}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-800 rounded-lg border border-red-500/40 p-5 max-w-md w-full mx-4 shadow-2xl">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-base font-semibold text-red-400">
                {t("syncPanel.deleteWarningTitle")}
              </h3>
            </div>
