...
interface YandexTrashManagerProps {
  onClose: () => void;
  onRefreshFiles?: () => void;
}
...
export function YandexTrashManager({ onClose, onRefreshFiles }: YandexTrashManagerProps) {
  ...
  const loadTrash = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await invoke<RemoteFile[]>('yandex_list_trash');
      setItems(result);
      setSelected(new Set());
    } catch (err) {
      setError(String(err));
  ...
  const getSelectedPaths = (): string[] => {
    return items.filter(item => selected.has(item.path)).map(item => item.path);
  };
