...
export function PCloudTrashManager({ onClose, onRefreshFiles }: PCloudTrashManagerProps) {
  ...
  const loadTrash = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await invoke<RemoteFile[]>('pcloud_list_trash');
      setItems(result);
      setSelected(new Set());
    } catch (err) {
      setError(String(err));
        ...
        {items.length > 0 && (
          <div className="flex items-center gap-2 px-4 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
            ...
            >
              ...
              {t('contextMenu.permanentDelete') || 'Delete'} {selected.size > 0 && `(${selected.size})`}
            </button>
            <button
