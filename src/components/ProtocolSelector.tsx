...
import { BoxLogo, PCloudLogo, AzureLogo, FilenLogo, FourSharedLogo, ZohoWorkDriveLogo, InternxtLogo, KDriveLogo, JottacloudLogo, DrimeCloudLogo, FileLuLogo, KoofrLogo, OpenDriveLogo, YandexDiskLogo, GitHubLogo, BlompLogo, FeliCloudLogo, ImmichLogo, ImageKitLogo, UploadcareLogo, CloudinaryLogo } from './ProviderLogos';
    ...
    return (
        <div className={className}>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                    ...
                    {PROTOCOLS.filter(p => !p.isCloudStorage && (!allowedProtocols || (allowedProtocols.includes(p.type) && !p.providerId))).map((protocol) => (
                        ...
                        >
                            ...
                            {protocol.badge && (
                                <span className={`text-[10px] px-1.5 py-0.5 rounded inline-flex items-center gap-0.5 flex-shrink-0 ${isSecureBadge(protocol.badge)
            ...
            {value && effectiveBadge && !isOpen && (
                <span className={`inline-flex items-center gap-1 mt-1.5 text-xs px-2 py-0.5 rounded-full ${isSecureBadge(effectiveBadge)
                        ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300'
