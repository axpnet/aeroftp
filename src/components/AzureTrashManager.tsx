...
interface AzureTrashManagerProps {
  onClose: () => void;
  onRefreshFiles?: () => void;
}
...
export function AzureTrashManager({ onClose, onRefreshFiles }: AzureTrashManagerProps) {
  ...
  const loadTrash = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await invoke<AzureTrashItem[]>('azure_list_deleted_blobs');
      setItems(result);
      setSelected(new Set());
    } catch (err) {
      setError(String(err));
  ...
  const toggleSelect = (item: AzureTrashItem) => {
    const id = item.path;
    setSelected(prev => {
  ...
  const handleRestore = async () => {
    ...
    const logId = humanLog.logRaw('activity.trash_restore_start', 'INFO', { provider: 'Azure Blob', count: selectedItems.length });
    ...
    try {
      ...
      humanLog.updateEntry(logId, { status: 'error', message: `[Azure Blob] Failed to restore from trash` });
      setError(String(err));
    } finally {
