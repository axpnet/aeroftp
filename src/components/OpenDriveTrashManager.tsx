...
interface OpenDriveTrashManagerProps {
  onClose: () => void;
  onRefreshFiles?: () => void;
}

interface OpenDriveTrashActionItem {
  itemId: string;
  isDir: boolean;
}
...
export function OpenDriveTrashManager({ onClose, onRefreshFiles }: OpenDriveTrashManagerProps) {
  ...
  const loadTrash = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await invoke<RemoteFile[]>('opendrive_list_trash');
      setItems(result);
      setSelected(new Set());
    } catch (err) {
      setError(String(err));
  ...
  const itemId = (item: RemoteFile) => item.metadata?.opendrive_item_id || item.path;
  ...
  const getSelectedItems = (): OpenDriveTrashActionItem[] => {
    return items
      .filter(item => selected.has(itemId(item)))
      .map(item => ({
        itemId: item.metadata?.opendrive_item_id || item.path,
        isDir: item.is_dir,
      }));
  };
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={t('contextMenu.opendriveTrashTitle')}
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2">
            <Trash2 size={18} className="text-cyan-500" />
            <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">
              {t('contextMenu.opendriveTrashTitle')}
            </h2>
        ...
        {items.length > 0 && (
          <div className="flex items-center gap-2 px-4 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1 text-xs rounded bg-cyan-600 text-white hover:bg-cyan-700 disabled:opacity-40 disabled:cursor-not-allowed"
            >
                  ...
                  return (
                    <tr
                      ...
                      className={`cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 border-b border-gray-200 dark:border-gray-700/30 ${
                        selected.has(id) ? 'bg-cyan-500/10' : ''
                      }`}
